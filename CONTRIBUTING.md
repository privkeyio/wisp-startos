...
wisp is compiled from source inside the `Dockerfile` (Zig), so the first build downloads the Zig toolchain and the wisp source; Docker layer-caches both, so later repacks are quick. For a complete list of build options, see [Makefile](https://docs.start9.com/packaging/makefile.html).
